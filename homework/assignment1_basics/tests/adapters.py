...
def run_linear(
    ...
    linear_model = Linear(d_in, d_out, device=device, dtype=weights.dtype)
    linear_model.weight = nn.Parameter(weights)
...
def run_embedding(
    ...
    """
    device = torch.device("gpu" if torch.cuda.is_available() else "cpu")
    embedding_model = Embedding(vocab_size, d_model, device=device, dtype=weights.dtype)
    embedding_model.weight = nn.Parameter(weights)
...
def run_multihead_self_attention(
    ...
    multihead_self_attention = CausalMultiHeadAttention(d_model, num_heads, max_seq_len, use_rope=False, device=device, dtype=dtype)
...
def run_multihead_self_attention_with_rope(
    ...
    device, dtype = in_features.device, in_features.dtype
    max_seq_len = in_features.shape[-2]
    multihead_self_attention = CausalMultiHeadAttention(d_model, num_heads, max_seq_len,theta, use_rope=(token_positions is not None), device=device,
                                                        dtype=dtype)
...
def run_transformer_block(
    ...
    transformer_block.causal_multi_head_attention.q_proj.weight.data.copy_(weights["attn.q_proj.weight"])
    transformer_block.causal_multi_head_attention.k_proj.weight.data.copy_(weights["attn.k_proj.weight"])
    transformer_block.causal_multi_head_attention.v_proj.weight.data.copy_(weights["attn.v_proj.weight"])
    transformer_block.causal_multi_head_attention.o_proj.weight.data.copy_(weights["attn.output_proj.weight"])
...
def run_rmsnorm(
    ...
    output = rms_norm.forward(in_features)
    return output
